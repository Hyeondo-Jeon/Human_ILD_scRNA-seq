...
Idents(Lym) <- Lym$cell
DimPlot(Lym, label = T, label.size = 5, repel = T) # 700 550
...
marker_list_lym <- c("PTPRC","MS4A1","CD19","BANK1","BLK","FCER2","TCL1A","MME","DTX1","MKI67","TOP2A","MZB1","JCHAIN","CD3E","CD4","FOXP3","CTLA4",
                     "IL7R","SELL","CCR7","CD8A","CCL5","GZMK","CD28","GZMB","PRF1","NKG7","NCAM1","FCGR3A","FGFBP2","GZMH","KIT","PCDH9")
DotPlot(Lym, features = marker_list_lym, cols = c("lightgray", "slateblue4"), group.by = "cell") + 
  ...
  theme(axis.text.x = ggtext::element_markdown(angle= 45,hjust = 1, size = 14),
        ...
        legend.text = element_text(size = 14)) + coord_flip() # 550 700
Lym_diag <- Lym@meta.data %>% group_by(diagnosis, cell) %>% summarise(n = n()) %>% ungroup()
ggplot(Lym_diag, aes(x = diagnosis, y = n, fill = cell)) + geom_bar(stat = "identity", position = "fill") + ylab("Proportion") + xlab(" ") +
  theme_classic(base_size = 18) +  labs(fill = " ") +  theme(axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black")) +
  scale_fill_manual(values = c("CD4+ Naive" = "#F8766D","CD4+ Treg" = "#E18A00","CD8+GZMK+ Memory" = "#BE9C00","CD56 bright NK"="#8CAB00","CD56 dim NK" = "#24B700","CD8+ CTL" = "#00BE70",
                               "CD8+IL7R+ Memory" ="#00C1AB","ILC" = "#00BBDA","Plasma" = "#00ACFC","Activated B" = "#8B93FF","Cycling B" ="#D575FE","Naive B"="#F962DD","GC B"="#FF65AC")) # 600 600
...
Idents(Mye) <- Mye$cell
DimPlot(Mye, label = T, label.size = 5, repel = T) # 700 500
...
marker_list_mye <- c("C1QA","C1QB","FABP4","MARCO","PPARG","MKI67","TOP2A","PCNA","SPP1","MERTK","LGALS3", "TREM2", "GPNMB", "LPL", "CD9",
                     "FCN1","CD14","FCGR3A","LYZ","S100A9","CLEC4C","IL3RA","TCF4","CD1C","FCER1A","ITGAX","CLEC9A","XCR1","BATF3",
                     "MS4A2","TPSAB1","KIT")
DotPlot(Mye, features = marker_list_mye, cols = c("lightgray", "slateblue4"), group.by = "cell") + 
  labs(x = " ", y = " ") +
  theme(axis.text.x = ggtext::element_markdown(angle= 45,hjust = 1, size = 14),
        axis.text.y = ggtext::element_markdown(hjust = 1, size = 14, face = "italic"),
        legend.text = element_text(size = 14)) + coord_flip()

Mye_diag <- Mye@meta.data %>% group_by(diagnosis, cell) %>% summarise(n = n()) %>% ungroup()
ggplot(Mye_diag, aes(x = diagnosis, y = n, fill = cell)) + geom_bar(stat = "identity", position = "fill") + ylab("Proportion") + xlab(" ") +
  theme_classic(base_size = 18) + theme(axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black")) +
  labs(fill = " ") + scale_fill_manual(values = c("SPP1+ MΦ" = "#F8766D","SPP1+ Mono" = "#DB8E00","cMono" = "#AEA200","cDC2"="#64B200","Cycling MΦ" = "#00BD5C",
                               "Alveolar MΦ" = "#00C1A7","pDC"="#00BADE","LAM" = "#00A6FF","Mast cell"="#B385FF","iMono"="#EF67EB","cDC1" = "#FF63B6"))
...
Stromal$cell <- factor(Stromal$cell,levels = c("PLIN2+ fibroblast","Fibroblast","FGFR4+ myofibroblast","SFRP4+ myofibroblast","POSTN+ myofibroblast",
                                               "Pericyte","SMC","Mesothelial cell"))
marker_list_str <- c("COL1A1","WT1","PLIN2","HAS2","C3","SFRP1","MGST1","LEPR","BMP5","COL8A1","WNT2","FGFR4","MECOM","SFRP4",
                     "COL3A1","POSTN","MOXD1","PDGFRB","RGS5","HIGD1B","ACTG2","ACTA2","CNN1","MYH11","UPK3B","MSLN")
...
Str_diag <- Stromal@meta.data %>% group_by(diagnosis, cell) %>% summarise(n = n()) %>% ungroup()
ggplot(Str_diag, aes(x = diagnosis, y = n, fill = cell)) + geom_bar(stat = "identity", position = "fill") + ylab("Proportion") + xlab(" ") +
  theme_classic(base_size = 18) + theme(axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black")) +
  labs(fill = " ") + scale_fill_manual(values = c("PLIN2+ fibroblast" = "#F8766D","Fibroblast" = "#CD9600","FGFR4+ myofibroblast" = "#7CAE00",
    "SFRP4+ myofibroblast" = "#00BE67","POSTN+ myofibroblast" = "#00BFC4","Pericyte" = "#00A9FF","SMC" = "#C77CFF", "Mesothelial cell" = "#FF61CC"))
...
Epithelial$cell <- factor(Epithelial$cell,levels = c("AT2","Transitional AT2","AT1","SCGB3A2+ Secretory","Club","Goblet","MUC5AC+ Goblet",
                                                     "Basal","KRT5- Basal","Cycling Epithelial","Ciliated"))
marker_gene_epi <- c("EPCAM","SFTPC","NAPSA","SFTPA1","AGER","CAV1","PDPN","SCGB3A2","SFTPB","SCGB3A1","SCGB1A1","TFF3","MUC5B",
                     "BPIFB1","MUC5AC","KRT5","KRT15","KRT17","CDK1","TOP2A","MKI67","FOXJ1","PIFO","TPPP3")
...
Epi_diag <- Epithelial@meta.data %>% group_by(diagnosis, cell) %>% summarise(n = n()) %>% ungroup()
ggplot(Epi_diag, aes(x = diagnosis, y = n, fill = cell)) + geom_bar(stat = "identity", position = "fill") + ylab("Proportion") + xlab(" ") +
  theme_classic(base_size = 18) +  theme(axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black")) + labs(fill = " ") +
  scale_fill_manual(values = c("SCGB3A2+ Secretory" = "#F8766D","Ciliated" = "#DB8E00","Transitional AT2" = "#AEA200","Goblet" = "#64B200","Club" = "#00BD5C","Basal" = "#00C1A7",
                               "MUC5AC+ Goblet" = "#00BADE","AT2" = "#00A6FF","AT1" = "#B385FF","Cycling Epithelial" = "#EF67EB","KRT5- Basal" = "#FF63B6"))
...
marker_gene_end <- c("HEY1","GJA4","GJA5","LYVE1","PROX1","GYPC","ACKR1","CCL14","SELE")
...
End_diag <- Endothelial@meta.data %>%  group_by(diagnosis, cell) %>% summarise(n = n()) %>%  ungroup()
ggplot(End_diag, aes(x = diagnosis, y = n, fill = cell)) + geom_bar(stat = "identity", position = "fill") + ylab("Proportion") + xlab(" ") +
  theme_classic(base_size = 18) +  theme(axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black")) + labs(fill = " ") +
  scale_fill_manual(values = c("Vein" = "#F8766D","Artery" = "#00BA38","Lymphatic" = "#619CFF"))
